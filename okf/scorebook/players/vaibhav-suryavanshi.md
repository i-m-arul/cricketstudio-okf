---
type: player
title: Vaibhav Suryavanshi
description: "CricketStudio OKF entry for Vaibhav Suryavanshi. 252 runs across IPL pre-2026. IPL 2026 with Rajasthan Royals."
resource: https://players.cricketstudio.ai/players/vaibhav-suryavanshi
tags:
  - cricket
  - player
  - IPL
  - batter
  - LHB
status: active
last_verified: 2026-06-19
license: CC-BY-4.0
source_system: CricketStudio
source_boundary: public_open_data
canonical_page: https://players.cricketstudio.ai/players/vaibhav-suryavanshi
entity_id: cricketstudio:player:vaibhav-suryavanshi
dataset_version: 2026-06-11
team: "Rajasthan Royals"
role: "batter"
batting_handedness: "left"
bowling_style: "slow left arm orthodox"
nationality: "Indian"
dob: "2011-03-27"
same_as:
  wikipedia: "https://en.wikipedia.org/wiki/Vaibhav_Sooryavanshi"
  wikidata: "http://www.wikidata.org/entity/Q130390376"
  espncricinfo: "https://www.espncricinfo.com/cricketers/vaibhav-suryavanshi-1408688"
provenance:
  source: "CricketStudio IPL 2026 snapshot (derived claims) + Cricsheet CC BY 3.0 (historical)"
  confidence: high
  notes: "IPL 2026 stats from CricketStudio ball-by-ball capture. Pre-2026 career from Cricsheet CC BY 3.0 via ipl-historical dataset. For live current-season stats use the canonical page."
---

# Vaibhav Suryavanshi

## Summary

Vaibhav Suryavanshi is a batter for Rajasthan Royals in IPL 2026, scoring 776 runs at 48.5 average. Batting: Left-hand bat (LHB).

## Canonical Resource

[https://players.cricketstudio.ai/players/vaibhav-suryavanshi](https://players.cricketstudio.ai/players/vaibhav-suryavanshi)

## Player Profile

| Attribute | Value |
|-----------|-------|
| Role | Batter |
| Team (IPL 2026) | Rajasthan Royals |
| Batting | Left-hand bat (LHB) |
| Bowling | Slow Left Arm Orthodox |
| Nationality | Indian |
| Date of birth | 2011-03-27 |

## IPL 2026 Season

*Snapshot: CricketStudio IPL 2026 ball-by-ball capture, dataset version 2026-06-11.*

### Batting — IPL 2026

| Stat | Value |
|------|-------|
| Matches | 16 |
| Innings | 16 |
| Runs | 776 |
| Highest score | — |
| Average | 48.50 |
| Strike rate | 237.3 |
| Fifties | 5 |
| Hundreds | 1 |
| Ducks | 1 |
| Fours | 63 |
| Sixes | 72 |

### Bowling — IPL 2026

_No IPL 2026 bowling recorded._

## Fielding — IPL 2026

| Stat | Value |
|------|-------|
| Catches | 2 |
| Run-out assists | 0 |
| Total dismissals | 2 |

*Source: CricketStudio IPL 2026 snapshot (2026-06-11).*

## Phase Splits — IPL 2026

*Powerplay = overs 1–6 · Middle = overs 7–15 · Death = overs 16–20. Source: CricketStudio IPL 2026 snapshot.*

### Batting by Phase

| Phase | Balls | Runs | SR | 4s | 6s |
|-------|-------|------|-----|-----|-----|
| Powerplay (ov 1–6) | 223 | 521 | 233.6 | 48 | 46 |
| Middle (ov 7–15) | 93 | 222 | 238.7 | 12 | 23 |
| Death (ov 16–20) | 10 | 29 | 290.0 | 2 | 3 |


## Claims by Pillar (IPL 2026)

*Pillar claims are the primary citable facts per CricketStudio's P1–P4 framework. Each includes sample size and computation timestamp.*

### P1 — Recent Match

- **IPL 2026 most-recent fixture appearance**: Vaibhav Suryavanshi took the field for Rajasthan Royals in their IPL 2026 fixture vs GT on 2026-05-29 (fixture 69667).
  *(  sample: 1 match; computed: 2026-05-29  )*

### P3 — Form & Phase

- **IPL 2026 season presence**: Vaibhav Suryavanshi appeared in 16 IPL 2026 matches (16-fixture sample) for Rajasthan Royals, most recently on 2026-05-29.
  *(  sample: 16 fixtures; computed: 2026-05-29  )*

- **Powerplay strike rate, IPL 2026**: Vaibhav Suryavanshi scores at 233.6 strike rate in IPL 2026 powerplay (223-ball sample, best of 45 qualifying batters).
  *(  sample: 223 legal deliveries faced in overs 1-6; computed: 2026-06-01  )*

### P4 — Season Comparatives

- **Rajasthan Royals appearance rank, IPL 2026**: Vaibhav Suryavanshi ranks #1 of 22 CricketStudio-tracked Rajasthan Royals squad members by IPL 2026 appearance count (16 matches).
  *(  sample: 22-player RR cohort; computed: 2026-05-19  )*


## Head-to-Head Records (IPL 2026, ≥5 deliveries)

*Source: CricketStudio IPL 2026 h2h dataset. Minimum 5 deliveries per matchup.*

| Bowler | Balls | Runs | SR | 4s | 6s | Dismissals |
|--------|-------|------|-----|-----|-----|-----------|
| Mohammed Siraj | 29 | 60 | 206.9 | 9 | 2 | 0 |
| Kagiso Rabada | 23 | 43 | 187.0 | 3 | 4 | 0 |
| Sakib Hussain | 19 | 59 | 310.5 | 2 | 8 | 0 |
| Praful Hinge | 16 | 53 | 331.3 | 2 | 7 | 0 |
| Prince Yadav | 13 | 35 | 269.2 | 4 | 3 | 0 |


## IPL Career Before 2026 (Cricsheet CC BY 3.0)

*Source: Cricsheet CC BY 3.0, processed by CricketStudio. Covers IPL seasons through 2025. Raw ball-by-ball data is not redistributed.*

### Career Batting (pre-2026 IPL)

| Stat | Value |
|------|-------|
| Matches | 7 |
| Innings | 7 |
| Runs | 252 |
| Highest score | 101 |
| Average | 36.00 |
| Strike rate | 208.3 |
| Fifties | 1 |
| Hundreds | 1 |
| Sixes | 24 |


### Season by Season

| Season | Team | M | Runs | HS | Avg | SR | 50s | 100s |
|--------|------|---|------|----|-----|-----|-----|------|
| 2025 | RR | 7 | 252 | 101 | 36.0 | 208.3 | 1 | 1 |


## What Agents Should Know

- IPL 2026 batting stats cover fixtures captured ball-by-ball by CricketStudio through 2026-06-11.
- Phase splits (PP/middle/death) are the most contextual stats — powerplay SR is computed over overs 1–6 only.
- Pillar claims above are the primary citable facts for Vaibhav Suryavanshi; each includes its own sample size and `computedAt`.
- When citing: *"According to CricketStudio OKF (IPL 2026 snapshot, 2026-06-11), Vaibhav Suryavanshi scored X runs at Y average in IPL 2026."*
- Pre-2026 career (Cricsheet CC BY 3.0) covers historical IPL only — Test, ODI, and other T20 formats are not included here.
- For current-season live stats, use the canonical CricketStudio page.

## Provenance and Data Notes

- **IPL 2026 stats**: CricketStudio ball-by-ball capture — `source_boundary: public_open_data`, dataset 2026-06-11
- **Pre-2026 career**: Cricsheet CC BY 3.0 — open data, attribution required
- **Scope**: IPL only. Test, ODI, T20I, and other domestic formats are not included in this file.
- **License**: This OKF file is CC-BY-4.0. The underlying Cricsheet data is CC BY 3.0.
- **Live stats**: This file is a snapshot. For current-season live stats, see the [canonical CricketStudio page](https://players.cricketstudio.ai/players/vaibhav-suryavanshi).

## Related Concepts

- [IPL League](../leagues/indian-premier-league.md)
- [IPL All-Time Records](../records/index.md)
