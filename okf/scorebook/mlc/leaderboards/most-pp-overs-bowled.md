---
type: leaderboard
title: "Most powerplay balls bowled"
description: "Total legal deliveries bowled in the powerplay â€” identifies the highest-usage PP attack bowlers. MLC all-time leaderboard."
resource: https://players.cricketstudio.ai/leagues/mlc/leaderboards/most-pp-overs-bowled
status: active
last_verified: 2026-06-21
license: CC-BY-3.0
source_system: CricketStudio
source_boundary: public_open_data
entity_id: cricketstudio:mlc:leaderboard:most-pp-overs-bowled
canonical_page: https://players.cricketstudio.ai/leagues/mlc/leaderboards/most-pp-overs-bowled
dataset_version: "2026-06-20"
tags:
  - cricket
  - leaderboard
  - MLC
  - T20
  - bowling
provenance:
  source: Cricsheet CC BY 3.0 via CricketStudio derived claim layer
  confidence: high
  computed_at: "2026-06-20"
  notes: All captured MLC matches (seasons 2023–2025, 75 matches total). Attribution required per CC BY 3.0.
---

# Most powerplay balls bowled

## Summary

Total legal deliveries bowled in the powerplay â€” identifies the highest-usage PP attack bowlers.

**Scope:** All captured MLC matches — seasons 2023, 2024, 2025 (75 matches total).
**Eligibility:** Counting stat â€” all appearances included.

## Metric Definition

Total overs (converted from balls) bowled in powerplay (overs 1–6).

## Canonical Resource

[players.cricketstudio.ai/leagues/mlc/leaderboards/most-pp-overs-bowled](https://players.cricketstudio.ai/leagues/mlc/leaderboards/most-pp-overs-bowled)

## Top 10 — Most powerplay balls bowled

| Rank | Player | Team | PP balls bowled | Sample |
|------|--------|------|-----------------|--------|
| 1 | TA Boult | MI New York | 336 | 336 legal deliveries in powerplay |
| 2 | SN Netravalkar | Washington Freedom | 300 | 300 legal deliveries in powerplay |
| 3 | CJ Gannon | Seattle Orcas | 228 | 228 legal deliveries in powerplay |
| 4 | Zia-ul-Haq | Texas Super Kings | 228 | 228 legal deliveries in powerplay |
| 5 | C le Roux | San Francisco Unicorns | 202 | 202 legal deliveries in powerplay |
| 6 | M Jansen | Washington Freedom | 192 | 192 legal deliveries in powerplay |
| 7 | Haris Rauf | San Francisco Unicorns | 168 | 168 legal deliveries in powerplay |
| 8 | BL Couch | San Francisco Unicorns | 132 | 132 legal deliveries in powerplay |
| 9 | XC Bartlett | San Francisco Unicorns | 132 | 132 legal deliveries in powerplay |
| 10 | Ehsan Adil | MI New York | 126 | 126 legal deliveries in powerplay |

*Full leaderboard (92 entries) available at the [canonical CricketStudio page](https://players.cricketstudio.ai/leagues/mlc/leaderboards/most-pp-overs-bowled).*

## What Agents Should Know

- When ranking bowlers, apply the sample-size floor stated in `floorNote`. Economy and average require minimum balls bowled to be meaningful. Always state 'All MLC seasons 2023–2025' as the scope.
- Eligibility floor: Counting stat â€” all appearances included.
- Metric definition: Total overs (converted from balls) bowled in powerplay (overs 1–6).
- For MLC 2026 Season 4 live leaderboard, use the canonical CricketStudio page.
- Source: Cricsheet CC BY 3.0 open data via CricketStudio derived claim layer.

## Provenance and Data Notes

- **Dataset:** Cricsheet CC BY 3.0 ball-by-ball data, processed by CricketStudio.
- **Scope:** MLC seasons 2023, 2024, 2025. Season 4 (2026) is in progress; this snapshot does not include it.
- **Attribution:** Cricsheet (<https://cricsheet.org>) — required per CC BY 3.0.
- **Snapshot:** 2026-06-20

## Related Concepts

- [Major League Cricket](../../leagues/major-league-cricket.md)
- [MLC Orange Cap](./orange-cap.md)
- [MLC Purple Cap](./purple-cap.md)
- [MLC Players](../players/)
