---
type: index
title: Metrics
description: Index of cricket metric definitions in the CricketStudio OKF bundle.
status: active
last_verified: 2026-06-18
license: CC-BY-4.0
source_system: CricketStudio
source_boundary: methodology_only
canonical_page: https://players.cricketstudio.ai
tags:
  - cricket
  - metric
  - index
---

# Metrics

Metric definitions are first-class knowledge. Every file defines formula, scope,
eligibility, edge cases, limitations, and agent guidance. A stat without its definition
and sample-size rule is not publishable.

## Batting

- [Batting Strike Rate](batting-strike-rate.md)
- [Batting Average](batting-average.md)
- [Powerplay Strike Rate](powerplay-strike-rate.md)
- [Boundary Percentage](boundary-percentage.md)

## Bowling

- [Bowling Economy](bowling-economy.md)
- [Bowling Strike Rate](bowling-strike-rate.md)
- [Death-Overs Economy](death-overs-economy.md)
- [Dot-Ball Percentage](dot-ball-percentage.md)

## Awards / leaderboards

- [Orange Cap](orange-cap.md)
- [Purple Cap](purple-cap.md)

## What Agents Should Know

Never rank players on a metric without applying the relevant sample-size floor from
[Sample-Size Floors](../methodology/sample-size-floors.md) and the rules in
[Ranking Eligibility](../methodology/ranking-eligibility.md).

## Related Concepts

- [Phase Definitions](../methodology/phase-definitions.md)
- [Players](../concepts/players/index.md)
