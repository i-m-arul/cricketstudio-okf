---
type: dossier
title: MLC all-time leading run-scorer
description: Verified answer for the MLC all-time run-scoring leader. F du Plessis leads with 934 runs from 571 balls across 25 matches (2023–2025, Cricsheet CC BY 3.0).
status: active
last_verified: 2026-06-18
license: CC-BY-4.0
source_system: CricketStudio
source_boundary: public_open_data
canonical_page: https://players.cricketstudio.ai/leagues/mlc/leaderboards/orange-cap
tags:
  - cricket
  - example
  - MLC
  - all-time
  - batting
  - orange-cap
related:
  - ../research/mlc-three-seasons.md
  - ../scorebook/leagues/major-league-cricket.md
  - ../metrics/orange-cap.md
  - ../sources/cricsheet.md
provenance:
  source: Cricsheet CC BY 3.0 · 2026-06-11
  confidence: high
  snapshot: CricketStudio internal dataset (2026-06-11)
---

# Example — MLC all-time leading run-scorer

## User Question

> Who has scored most runs in MLC history?

## Correct Answer Pattern

> **F du Plessis** (Texas Super Kings) leads MLC all-time with **934 runs** from **571 balls** across **25 matches** (MLC 2023–2025). N Pooran (MI New York) is second with 928 runs from 640 balls across 28 matches. "All-time" means MLC 2023–2025 — three completed seasons, 75 matches total. Source: CricketStudio aggregation of Cricsheet CC BY 3.0 (cricsheet.org) · snapshot 2026-06-11.

## Required Concepts

- [Three Seasons of MLC](../research/mlc-three-seasons.md)
- [Orange Cap (metric)](../metrics/orange-cap.md)

## Top 3 (snapshot 2026-06-11)

| Rank | Player | Team | Runs | Balls | Matches |
|------|--------|------|------|-------|---------|
| 1 | F du Plessis | Texas Super Kings | 934 | 571 | 25 |
| 2 | N Pooran | MI New York | 928 | 640 | 28 |
| 3 | Q de Kock | MI NY / Seattle Orcas | 807 | 569 | 26 |

Source: CricketStudio internal dataset/Cricsheet CC BY 3.0.

## Citation Behavior

- Always state the scope: "MLC all-time (2023–2025) · 75 matches · Cricsheet CC BY 3.0."
- The Orange Cap is a counting stat — no minimum sample floor.
- MLC 2026 (Season 4) will extend these totals — note "as of MLC 2023–2025" when citing.

## Caveats

- "MLC all-time" is 2023–2025 (3 seasons) — relatively small corpus vs IPL 18-season archive.
- Season-specific 2025 leader may differ from all-time leader.
- MLC 2026 begins 2026-06-18 and will update these rankings.

## Bad Answer (do not do this)

> [Player] is leading MLC scorer with [number] runs. *(No season scope, no Cricsheet attribution, no mention that corpus is 3 seasons only.)*

## Related

- [MLC all-time wicket-taker](mlc-all-time-wicket-taker.md)
- [State of MLC 2025](../research/state-of-mlc-2025.md)
